      ...
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription className="text-gray-400 mt-1">
            Manage your entire real estate portfolio from this centralized dashboard.
          </CardDescription>
        </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-b-white/10">
              ...
              <TableHead className="text-white/80 text-center">Status</TableHead>
              <TableHead className="text-white/80 text-right">Monthly Rate (ZAR)</TableHead>
              <TableHead className="text-white/80 text-center">Actions</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {sampleProperties.map((prop) => (
              <TableRow key={prop.id} className="border-b-white/10 hover:bg-white/5">
                <TableCell className="font-mono text-sm text-gray-300">{prop.id}</TableCell>
                <TableCell className="font-medium text-white">{prop.title}</TableCell>
                <TableCell className="text-gray-400">{prop.location}</TableCell>
                <TableCell className="text-center">
                  <Badge variant="outline" className={`font-semibold ${getStatusBadge(prop.status)}`}>
                    {prop.status}
                  </Badge>
                </TableCell>
                <TableCell className="text-right font-semibold text-white">R {prop.monthly_rate.toLocaleString()}</TableCell>
                <TableCell className="text-center">
                    <Link href={`/dashboard/properties/${prop.id}`}>
                        <Button variant="outline" size="sm" className="border-brand-gold/50 text-brand-gold hover:bg-brand-gold hover:text-brand-navy font-bold">
